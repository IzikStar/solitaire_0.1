...
const Solitaire = () => {
  ...
  const [pileArray, setPileArray] = useState([pile1, pile2, pile3, pile4]);
...
useEffect(() => {
  if (selectedCard !== null) {
    console.log("selectedCard:", selectedCard);  // הדפסה לבדיקת ה- selectedCard
    const card = deck.filter(c => c.code === selectedCard)[0];
    console.log("card from deck:", card);  // הדפסה לבדיקת הקלף שנבחר מהחפיסה
    ...
    console.log("fromStack:", fromStack);  // הדפסה לבדיקת fromStack
    console.log("selectedCards from stack:", selectedCards);  // הדפסה לבדיקת הקלפים שנבחרו מהסטקים
      ...
      if (pileArray[i].length > 0) {
        for (let j = 0; j < pileArray[i].length; j++) {
          if (pileArray[i][j].code === selectedCard) { 
            fromPile[i] = true; 
            break; 
          } else fromPile[i] = false;
        }
        if (fromPile[i] === true) {
          selectedCards = pileArray[i].filter(c => pileArray[i].indexOf(c) === pileArray[i].indexOf(card))
        }
      }
    }
    console.log("fromPile:", fromPile);  // הדפסה לבדיקת fromPile
    console.log("selectedCards from pile:", selectedCards);  // הדפסה לבדיקת הקלפים שנבחרו מהפיילים
    ...
    for (let i = 0; i < jackpotCards.length; i++) {
      if (jackpotCards[i].code === selectedCard) { 
        fromJackpot = true; 
        break; 
      } else fromJackpot = false;
    }
    console.log("fromJackpot:", fromJackpot);  // הדפסה לבדיקת האם הקלף נלקח מה- jackpot
    ...
    console.log("toStack:", toStack);  // הדפסה לבדיקת הסטקים שאפשר להעביר אליהם

    // בדיקה האם הקלף שנבחר תקף להעברה לפייל כלשהו
    for (let i = 0; i < pileArray.length; i++) {
      if (pileArray[i].length > 0) {
        if (isValidToPilesMove(selectedCard, pileArray[i][0].code)) 
          toPile[i] = true;
        else 
          toPile[i] = false;
      } else 
    ...
    console.log("toPile:", toPile);  // הדפסה לבדיקת הפיילים שאפשר להעביר אליהם

    // תנועות לקלפים בתוך הפייל
    switch (getRandomIndex(toPile, true)) {
      case 0:
        setPile1(prevPile => [...selectedCards, ...prevPile]);
        break;
      case 1:
        setPile2(prevPile => [...selectedCards, ...prevPile]);
        break;
      case 2:
        setPile3(prevPile => [...selectedCards, ...prevPile]);
        break;
      case 3:
        setPile4(prevPile => [...selectedCards, ...prevPile]);
        break;
    }
    ...
    console.log("Updated stacks and piles after move");  // הדפסה לבדיקת מצב הסטקים והפיילים לאחר תנועה
    ...
    console.log("Stacks, Piles and Jackpot updated after move");  // הדפסה לבדיקת הסטקים, הפיילים וה- jackpot לאחר ניקוי
  }
}, [numOfClicks]);
